...
def generate_docx(output_path="paper_latex/Research_Article_Journal_of_Hydroinformatics.docx", journal="hydroinformatics"):
    ...
    p_abs = doc.add_paragraph(
        "Accurate monthly streamflow forecasting in managed alluvial basins remains challenging due to hydro-climatic non-stationarity and observational scarcity. While machine learning captures non-linear hydrological processes, unconstrained models suffer from physical inconsistency and peak flow dampening under out-of-distribution extremes. We present a Physics-Anchored Water-Balance Hybrid Hydroinformatics Framework (PG-MCH) for the Lower Indus Basin within Sindh, Pakistan (140,914 km²). The model couples a non-negative, L2-regularized linear water-balance baseline with multi-sensor residual gradient boosted decision trees and dual physical mass bounds. The framework ingests 2000–2024 satellite Earth observations (CHIRPS, ERA5-Land, SMAP, GRACE) alongside upstream Guddu Barrage telemetry (N = 300 months). Antecedent memory is formulated strictly as lagged storage (APIt = γ Pt-1 + γ² Pt-2), eliminating precipitation double-counting within an upper mass envelope. Calibrated on a 20-year baseline (2000–2019) and evaluated on the unseen 2022 Pakistan Mega-Flood, PG-MCH achieved KGE = 0.812 and restricted peak underestimation bias (FHV) to -4.0%, whereas unconstrained GBDT collapsed (FHV = -13.3%). Across 2020–2024 out-of-sample testing (N = 60), PG-MCH maintained KGE = 0.895 [95% CI: 0.73, 0.93], NSE = 0.838, and RMSE = 11.78 mm/month. Physical baselines effectively prevent peak collapse during extreme hydrometeorological shocks."
    )
